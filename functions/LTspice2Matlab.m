function data = LTspice2Matlab( filename, varargin )
%
%
...
while 1
    ...
    if length(the_line)==1 && isnumeric(the_line) && double(the_line)==-1
        ...
        error(['Format error in LTspice file ' filename ' ... End of file unexpectedly encountered']);
    end
    ...
    if variable_flag==0  %Non-variable header section
        ...
        if isempty(colon_index)
            ...
            error(['Format error in LTspice file ' filename ]);
        end
        ...
        if isempty(var_name) || (var_name(1)>='0' && var_name(1)<='9')
            ...
            error(['Format error in LTspice file ' filename ' ... Bad tag name found']);
        end
        
        if strcmpi( var_name, 'variables' ) || strcmpi( var_name, 'variable' ),  variable_flag = 1;  continue;  end
        value_try = str2num(var_value);
        try
            if isempty(value_try)
                raw_data = setfield( raw_data, var_name, var_value );
            else
                raw_data = setfield( raw_data, var_name, value_try );  
            end
        catch
...
elseif strcmpi(file_format, 'ascii' )
    ...
    elseif strcmpi( simulation_type, '.tran' )
    ...
    if downsamp_N==1
        ...
        raw_data.conversion_notes = ['Converted from ASCII format.  Downsampled from %.0f to %.0f points' NumPnts NumPnts_DS ];  
    end
...
%% Convert to Simview data format

if isfield( raw_data, 'time_vect' )
    data.time=raw_data.time_vect';
    data.Ts=data.time(2,1)-data.time(1,1); % Time step
    
    % Verifies header name
    for i=1:raw_data.num_variables
        if verLessThan('matlab', '8.2.0')
            U = genvarname(raw_data.variable_name_list{i});
        else
            [U, ~] = matlab.lang.makeValidName(raw_data.variable_name_list{i},'ReplacementStyle','delete');
        end
        
        data.signals(i).label=U;
        
        data.signals(i).values=raw_data.variable_mat(i,:)';
        data.signals(i).mean=mean(data.signals(i).values);
        data.signals(i).rms=rms(data.signals(i).values);
        data.signals(i).dimensions=1;
        data.signals(i).title=raw_data.variable_type_list{i};
        data.signals(i).plotStyle=[0,0];
    end
    
    data.blockName=raw_data.plotname;
