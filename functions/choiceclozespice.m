...
function quizstruct = choiceclozespice(quizstruct)
...
for q=1:length(nq)
    n=nq(q);
    parstr=param2str(quizstruct.Xu(1:length(quizstruct.parname),n),quizstruct.parname,quizstruct.parunit);
    ...
    tablebody1{1,1}='Médio';%
    for a=2:length(quizstruct.tableheader1)
        tablebody1{1,a}=clozeVmedido(quizstruct.Ru{n}(quizstruct.question.nopts1(a)),quizstruct.vtol);% 
    end
    [tabletext1]=clozetabgen(tablebody1,quizstruct.tableheader1); % 
    
    tablebody2{1,1}='Médio';%
    for b=2:length(quizstruct.tableheader2)
        tablebody2{1,b}=clozeXcalc(quizstruct.Ru2{n}(quizstruct.question.nopts2(b)),quizstruct.vtol,quizstruct.mults(b));% Vsec
    end
    [tabletext2]=clozetabgen(tablebody2,quizstruct.tableheader2); % Parece funcionar   
  
    
      
    questionstr=['<p>Monte o circuito indicado na Figura 01 e faça as medições e cálculos indicados nas tabelas.'  figstr '<br></p>'];
    quizstruct.question.text{q}=[ questionstr ' <p>'  tabletext1  'Tabela 1: Grandezas medidas com multímetro digital.<br></p><p>' tabletext2 'Tabela 2: Grandezas calculadas.<br></p>' ];
