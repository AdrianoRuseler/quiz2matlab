...
circuit.name = 'TBJ06a'; % File name
circuit.dir = getsimdir([circuit.name '.m'],'LTspice'); % Sets simulation dir
circuit.theme  = 'boost'; % clean or boost

% Config simulation
circuit.parnamesim={'Vcc','Rc','R1','Re','R2'}; % Variables names
circuit.parname={'Vcc','Rc','R1','Re','R2'}; % Variables names
...
circuit.Xi=CombVec(Rc,R1);

circuit.Xi(3,:)=circuit.Xi(1,:);
...
quiz.tablequestion{t}='Análog ao multímetro digital no modo CC:';
c=1;
...
quiz.table{t,2}.header = 'Polarização BE';
...
quiz.table{t,3}.header = 'Polarização BC';
quiz.table{t,3}.units='V';
...
quiz.tablecaption{3}='Tabela 3: Grandezas medidas no LTspice (Valor médio)!';
quiz.tablequestion{3}='Medição no LTspice, análogo ao multímetro digital no modo CC:';
